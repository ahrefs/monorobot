...
| `allowed_pipelines` | a list of allowed pipelines objects. Check the table below for the available configurations. | all pipelines included in the status rule check |

#### Allowed pipelines config

| key | description | default |
|-|-|-|
| `name` | the name of the pipeline to enable handling status notifications in | required field
| `failed_builds_channel` | the slack channel where failed build notifications will be sent. When specified, Monorobot will notify this channel about builds that fail with newly failed steps on the main branch. This is in addition to any notifications generated by the **status rules**. Note that if this is specified, builds from pipelines not in the allowed list won't trigger notifications when they fail. | empty
