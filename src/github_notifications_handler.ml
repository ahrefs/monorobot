open Httpaf
open Base
open Github_events_notifications_j
...
let is_valid_signature headers_sig body =
  let secret = Lazy.force Configuration.Env.github_webhook_secret_token in
  let request_hash =
...
let parse_exn headers body =
  ...
  | req_sig when not @@ is_valid_signature req_sig body -> Error "request signature invalid"
  | _ ->
  ...
  | "check_suite" -> Ok (CI_run (ci_build_notification_of_string body))
